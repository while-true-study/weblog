...
import { postApi } from "../services/api"; // postApi에 searchPosts 메서드 추가할 예정
...
const SearchPage: React.FC = () => {
  ...
  // prefix/infix 선택(기본은 infix로)
  const [mode, setMode] = useState<"infix" | "prefix">("infix");
  ...
  const fetchSearch = async (
    ...
    try {
      ...
      // postApi.searchPosts가 axios 기반이면 signal 전달은 axios 버전에 따라 다릅니다.
      // axios 1.x는 { signal } 지원합니다.
  ...
  // velog 느낌: 타이핑 멈추면 자동 검색 + URL q 갱신(replace)
  useEffect(() => {
    const keyword = q.trim();

    // URL도 같이 동기화(히스토리 쌓이지 않게 replace)
    if (!keyword) {
